...
function Blur2(blur) {
  return function(data, rx, ry = rx) {
    ...
    var {data: values, width, height} = data;
...
class InternMap extends Map {
  constructor(entries, key = keyof) {
    ...
    if (entries != null) for (var [key, value] of entries) this.set(key, value);
  }
  get(key) {
...
class InternSet extends Set {
  constructor(values, key = keyof) {
    super();
    Object.defineProperties(this, {_intern: {value: new Map()}, _key: {value: key}});
    if (values != null) for (var value of values) this.add(value);
  }
  has(value) {
...
function sort(values, ...F) {
  ...
  if ((f && f.length !== 2) || F.length > 1) {
    var index = Uint32Array.from(values, (d, i) => i);
    if (F.length > 1) {
...
function ticks(start, stop, count) {
  ...
  var reverse = stop < start, [i1, i2, inc] = reverse ? tickSpec(stop, start, count) : tickSpec(start, stop, count);
  if (!(i2 >= i1)) return [];
  var n = i2 - i1 + 1, ticks = new Array(n);
  if (reverse) {
    if (inc < 0) for (var i = 0; i < n; ++i) ticks[i] = (i2 - i) / -inc;
    else for (var i = 0; i < n; ++i) ticks[i] = (i2 - i) * inc;
  } else {
    if (inc < 0) for (var i = 0; i < n; ++i) ticks[i] = (i1 + i) / -inc;
    else for (var i = 0; i < n; ++i) ticks[i] = (i1 + i) * inc;
  }
  return ticks;
}
...
function thresholdFreedmanDiaconis(values, min, max) {
  var c = count$1(values), d = quantile$1(values, 0.75) - quantile$1(values, 0.25);
  return c && d ? Math.ceil((max - min) / (2 * d * Math.pow(c, -1 / 3))) : 1;
}
function thresholdScott(values, min, max) {
  var c = count$1(values), d = deviation(values);
...
function disjoint(values, other) {
  var iterator = other[Symbol.iterator](), set = new InternSet();
  for (var v of values) {
...
var {abs: abs$3, max: max$2, min: min$1} = Math;
function number1(e) {
  return [+e[0], +e[1]];
}
function number2(e) {
...
function chord$1(directed, transpose) {
  ...
  function chord(matrix) {
    ...
    // Compute the angles for each group and constituent chord.
    {
      ...
      for (var i of groupIndex) {
        var x0 = x;
        if (directed) {
          var subgroupIndex = range$1(~n + 1, n).filter(j => j < 0 ? matrix[~j * n + i] : matrix[i * n + j]);
          if (sortSubgroups) subgroupIndex.sort((a, b) => sortSubgroups(a < 0 ? -matrix[~a * n + i] : matrix[i * n + a], b < 0 ? -matrix[~b * n + i] : matrix[i * n + b]));
          for (var j of subgroupIndex) {
            if (j < 0) {
              var chord = chords[~j * n + i] || (chords[~j * n + i] = {source: null, target: null});
              chord.target = {index: i, startAngle: x, endAngle: x += matrix[~j * n + i] * k, value: matrix[~j * n + i]};
            } else {
              var chord = chords[i * n + j] || (chords[i * n + j] = {source: null, target: null});
              chord.source = {index: i, startAngle: x, endAngle: x += matrix[i * n + j] * k, value: matrix[i * n + j]};
            }
          }
          groups[i] = {index: i, startAngle: x0, endAngle: x, value: groupSums[i]};
        } else {
          var subgroupIndex = range$1(0, n).filter(j => matrix[i * n + j] || matrix[j * n + i]);
...
function smooth1(x, v0, v1, value) {
  var a = value - v0;
  var b = v1 - v0;
  var d = isFinite(a) || isFinite(b) ? a / b : Math.sign(a) / Math.sign(b);
...
class Delaunator {
    ...
    update() {
        var {coords, _hullPrev: hullPrev, _hullNext: hullNext, _hullTri: hullTri, _hullHash: hullHash} =  this;
        var n = coords.length >> 1;
        ...
        for (var i = 0, minDist = Infinity; i < n; i++) {
    ...
    _legalize(a) {
        var {_triangles: triangles, _halfedges: halfedges, coords} = this;
        var i = 0;
        var ar = 0;
...
class Voronoi {
  ...
  _init() {
    var {delaunay: {points, hull, triangles}, vectors} = this;
    ...
    for (var i = 0, j = 0, n = triangles.length, x, y; i < n; i += 3, j += 2) {
      ...
      if (Math.abs(ab) < 1e-9) {
        ...
        if (bx === undefined) {
          bx = by = 0;
          for (var i of hull) bx += points[i * 2], by += points[i * 2 + 1];
          bx /= hull.length, by /= hull.length;
        }
        var a = 1e9 * Math.sign((bx - x1) * ey - (by - y1) * ex);
        x = (x1 + x3) / 2 - a * ey;
        y = (y1 + y3) / 2 + a * ex;
      } else {
        var d = 1 / ab;
  ...
  render(context) {
    ...
    var {delaunay: {halfedges, inedges, hull}, circumcenters, vectors} = this;
  ...
  *neighbors(i) {
    var ci = this._clip(i);
    if (ci) for (var j of this.delaunay.neighbors(i)) {
      var cj = this._clip(j);
      // find the common edge
      if (cj) loop: for (var ai = 0, li = ci.length; ai < li; ai += 2) {
        for (var aj = 0, lj = cj.length; aj < lj; aj += 2) {
          if (ci[ai] === cj[aj]
              && ci[ai + 1] === cj[aj + 1]
  ...
  _cell(i) {
    var {circumcenters, delaunay: {inedges, halfedges, triangles}} = this;
    var e0 = inedges[i];
  ...
  _simplify(P) {
    if (P && P.length > 4) {
      for (var i = 0; i < P.length; i+= 2) {
        var j = (i + 2) % P.length, k = (i + 4) % P.length;
...
class Delaunay {
  ...
  _init() {
    ...
    if (d.hull && d.hull.length > 2 && collinear(d)) {
      ...
      var e = this.collinear[0], f = this.collinear[this.collinear.length - 1],
  ...
  *neighbors(i) {
    var {inedges, hull, _hullIndex, halfedges, triangles, collinear} = this;
  ...
  _step(i, x, y) {
    var {inedges, hull, _hullIndex, halfedges, triangles, points} = this;
  ...
  render(context) {
    var buffer = context == null ? context = new Path : undefined;
    var {points, halfedges, triangles} = this;
    for (var i = 0, n = halfedges.length; i < n; ++i) {
      var j = halfedges[i];
      if (j < i) continue;
...
function stratify() {
  ...
  function stratify(data) {
    var nodes = Array.from(data),
        ...
        nodeByKey = new Map;
    if (path != null) {
      var I = nodes.map((d, i) => normalize$1(path(d, i, data)));
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function tickInterval(start, stop, count) {
    ...
    var i = bisector(([,, step]) => step).right(tickIntervals, target);
    if (i === tickIntervals.length) return year.every(tickStep(start / durationYear, stop / durationYear, count));
    if (i === 0) return millisecond.every(Math.max(tickStep(start, stop, count), 1));
    var [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
...
var plus = {
  draw(context, size) {
    var r = sqrt(size - min(size / 7, 2)) * 0.87559;
    context.moveTo(-r, 0);
    context.lineTo(r, 0);
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var times = {
  draw(context, size) {
    var r = sqrt(size - min(size / 6, 1.7)) * 0.6189;
    context.moveTo(-r, -r);
    context.lineTo(r, r);
    context.moveTo(-r, r);
    context.lineTo(r, -r);
